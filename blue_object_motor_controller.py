"""Track a blue object and keep it centered using differential motor speeds.
...
commands to the Arduino to drive both motors in the same direction but at
different speeds so the vehicle arcs toward the object while moving forward or
backward.
"""
...
def main():
    ...
    base_speed = 150       # Baseline motor speed (0-255)
    turn_scale = 100       # How strongly to adjust speeds for steering
    last_left = 0
    last_right = 0
    ...
    while True:
        ...
        if contours:
            ...
            if abs(delta_y) > center_threshold:
                # Determine base speed and direction. The sign sets forward/backward
                # while the magnitude is always positive so both motors move the same
                # direction and only differ by speed.
                direction = -1 if delta_y < 0 else 1  # forward if object above center
                adjust = 0
                if abs(delta_x) > center_threshold:
                    # adjust is positive when the object is to the right of center
                    # and negative when to the left.
                    adjust = (delta_x / (frame_w / 2)) * turn_scale

                left = int(np.clip(base_speed + adjust, 0, 255))
                right = int(np.clip(base_speed - adjust, 0, 255))
                left *= direction
                right *= direction
        ...
        # Only send when speeds change to avoid flooding the serial connection.
